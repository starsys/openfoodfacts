...
class Terminal_interface:
    ...
    def print_sql(self):
        ...
        choice_category = self.user_menu("Veuillez choisir une catégorie en saisissant son ID puis entrée : ", "SELECT * FROM category; SELECT %s", ("",))

        category_name = make_query("SELECT name FROM category WHERE id = %s", (choice_category,))[0][0]

        choice_product = self.user_menu("Veuillez choisir un produit à substituer en saisissant son ID puis entrée : ", "SELECT * FROM product WHERE category_ID = %s", (choice_category,), "long")

        nutrition_grade_selected_product = make_query("SELECT nutrition_grades FROM product WHERE id = %s", (choice_product,))[0][0]

        product_name = make_query("SELECT name FROM product WHERE id = %s", (choice_product,))[0][0]
        if not make_query("SELECT * FROM product WHERE nutrition_grades < %s AND category_ID = %s", (nutrition_grade_selected_product, choice_category)):
            ...
            choice_substituted = self.user_menu("Si vous souhaitez enregistrer l'un de ces produit en substitution, tapez son ID, sinon, tapez 'q' pour quitter : ", "SELECT * FROM product WHERE nutrition_grades < %s AND category_ID = %s", (nutrition_grade_selected_product, choice_category), "long")
